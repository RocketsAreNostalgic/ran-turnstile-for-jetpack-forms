...
test('release entry point is the pinned Profile B wrapper for main Quality', () => {
	...
	assert.match(release, /uses: RocketsAreNostalgic\/\.github\/\.github\/workflows\/release-profile-b\.yml@e2fb19244a301a62f8fae2a80536898adf21fe22/);
	for (const input of [
...
test('WordPress.org observer admits only the exact successful release workflow', () => {
	...
	assert.deepEqual(
		predicate.split('&&').map((term) => term.trim()).sort(),
		[
	...
	assert.match(contract, /ref: \$\{\{ github\.event\.workflow_run\.head_sha \}\}/);
	assert.match(contract, /test "\$\(git rev-parse HEAD\)" = "\$RAN_ADMITTED_SHA"/);
	assert.match(contract, /\.target_commitish == \$sha and \.draft == false and \.immutable == true/);
	assert.match(contract, /\.object\.type == "commit" and \.object\.sha == \$sha/);
	assert.match(contract, /\.enabled \| type == "boolean"/);
	assert.match(contract, /if \[\[ "\$count" == 0 \]\]; then echo 'deploy-required=false'/);
});
...
test('SVN execution requires enabled policy and exact immutable two-asset readback', () => {
	...
	assert.match(deploy, /if: \$\{\{ needs\.contract\.outputs\.deploy-required == 'true' && needs\.contract\.outputs\.enabled == 'true' \}\}/);
	...
	assert.match(deploy, /\.target_commitish == \$commit and \.draft == false and \.immutable == true/);
